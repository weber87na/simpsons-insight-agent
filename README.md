...
2. 勾選來源並補上平台必要設定：Google Maps 選定分店、PTT 指定看板；Dcard 提供公開文章 URL／匯入批次。
...
- PTT：只讀取公開網頁版的指定看板，以關鍵字搜尋標題；固定單連線、每秒最多一個請求並
  遵循 `Retry-After`。不繞過 403，不蒐集會導向年齡確認的限制級看板。預設 50 篇文章、
  500 則推文、每串最多 100 則；硬上限為 200／2,000。
- Dcard：只接受使用者指定的 `https://www.dcard.tw/f/{forum}/p/{id}` 公開文章 URL，或
  CSV／JSON 匯入。不登入、不搜尋全站、不自動翻頁、不呼叫未公開 API。跨網域、非 HTTPS
  或非標準連接埠重新導向會被拒絕。公開頁無法提供完整留言時會標記 `PARTIAL`。
...
鍵。檔案最大 10 MB；任何一列有錯都回傳 HTTP 422，不會靜默略過。
...
報告 payload 為 `schema_version: 2`，提供整體及各來源統計、內容類型組成、來源蒐集狀態、
