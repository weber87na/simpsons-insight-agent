...
本文件下方保留最初構想作歷史參考，來源與技術選型請以 README 及現有程式碼為準。

已實作：跨平台報告快照、週／月趨勢、語意抱怨主題、四角色決策協作、版本化專家規則、相對／日期排程、內部工作回填、真人評估與品牌比較。沒有新增定期爬取或外部派工。

可重現驗證：合成三品牌種子資料、離線驗收測試、基準比較與匿名評分包工具。

尚待人工：OpView 試用帳號申請與實測、三位專家盲評及真實資料的決策效益驗證；不將這些列為完成。

詳細設計、API 與使用流程見 [決策升級文件](docs/decision-upgrade.md)；標竿紀錄見 [OpView 研究](docs/opview-benchmark.md)。
...
```powershell
uv init --package simpsons-insight-agent
cd simpsons-insight-agent
uv python pin 3.12
...
```text
simpsons-insight-agent/
├── pyproject.toml
├── uv.lock
├── src/simpsons_insight_agent/
│   ├── api.py
│   ├── config.py
│   ├── domain/
│   │   ├── models.py
